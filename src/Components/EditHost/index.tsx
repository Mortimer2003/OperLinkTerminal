import {
  ...
  useColorScheme,
  Dimensions, Modal, FlatList, ImageBackground, Easing, Animated, Keyboard, LayoutAnimation, KeyboardAvoidingView
} from "react-native";
import * as React from "react";
import { useContext, useEffect, useLayoutEffect, useRef, useState } from "react";
import { Colors } from "react-native/Libraries/NewAppScreen";
import { HostContext, saveData, UserContext } from "../../../App";
import { header } from "../Header";
import { createConnect } from "../../../module/httpModule/http";
import { HostType } from "../../../module/dataModule/types";

// @ts-ignore
export function EditHost({ navigation, target }) {
  ...
  const headerTitle = () => <Text style={styles.sectionTitle}>{target?"编辑主机":"新建主机"}</Text>;
  ...
  const [combine, onChangeCombine] = useState("");
  const [username, onChangeUsername] = useState("");
  const [host, onChangeHost] = useState("");
  ...
  let informations=[
    ...
    {
      value: host,
      onChangeValue: onChangeHost,
      text: "主机"
    },
    {
      value: port,
      onChangeValue: onChangePort,
      text: "端口"
    },
    {
      value: password,
  ...
  useLayoutEffect(()=>{
    if(target){
      onChangeCombine(target.item.username+"@"+target.item.host+":"+target.item.port)
      onChangeUsername(target.item.username)
      onChangeHost(target.item.host)
      onChangePort(target.item.port)
      onChangePassword(target.item.password)
      onChangeNickname(target.item.nickname)
      setProtocol(target.item.protocol)
      setEncode(target.item.encode)
    }
  },[])
  ...
  const [isMenuVisible_Setting, setIsMenuVisible_Setting] = useState(true);
  ...
  const handleSave = () => {
    if(!userSlice.enterprise){alert('暂不支持非企业用户创建！');return;}

    const regex = /^[a-zA-Z0-9._-]+@[a-zA-Z0-9.-]+:\d+$/;

    const newHost:HostType={
      connecting: false, time: null,  /*username, host, port,*/ password, nickname, encode, openShell, protocol,
      username:'', host:'', port:'',
    ...
    if(!password) {
      alert('请设置密码！');
      setIsMenuVisible_Setting(true)
      return;
    }

    console.log({combine,username,host,port})

    if (!isMenuVisible_Setting){
      if(!combine) {alert('请输入“用户名@主机:端口”！');return;}
      if(!regex.test(combine)) {alert('请输入正确格式的“用户名@主机:端口”！');return;}
      const parts = combine.split(/[@:]/);
      newHost.username=parts[0];
      newHost.host=parts[1];
      newHost.port=parts[2];
    }
    else{
    ...
    //新建连接
    createConnect(newHost.host,Number(newHost.port),newHost.password,newHost.username/*userSlice.nickname*/,userSlice.enterprise.id, newHost.nickname||'')
      .then((res)=>{
        if(res){
          //TODO: 刷新获取主机列表
    ...
    <SafeAreaView style={backgroundStyle}>
      ...
      <View style={backgroundStyle}>
        <ImageBackground
          ...
          style={{ flex: 1}}
        >
          <View style={[styles.body,/*isKeyboardOpen?{marginBottom: 100}:*/{marginBottom: 120,}, ]}>
            ...
            { !isKeyboardOpen&&<View style={{
              ...
              <TouchableOpacity
                ...
                style={[styles.button]}
              >
                <Text style={{ textAlign: "center", lineHeight: 50, fontSize: 20 }}>测试连接</Text>
              </TouchableOpacity>
              <TouchableOpacity
                activeOpacity={0.5}
                onPress={handleSave}
                style={[styles.button]}
              >
                <Text style={{ textAlign: "center", lineHeight: 50, fontSize: 20 }}>确认</Text>
              </TouchableOpacity>
            </View>}
          </View>
